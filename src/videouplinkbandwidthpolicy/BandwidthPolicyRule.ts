...
export default class BandwidthPolicyRule {
	...
	It is considered a match if all the following conditions apply:
		...
		2. The bitrate given is LESS THAN OR EQUAL TO the bitrate defined in the policy
	These two facts combined allow us to construct bandwidth policies as a list that
	goes from smallest numbers of participants and worst case bandwidth up. (or put it another
	way. Start from the bottom, and compare numbers, discarding till the rule exceeds our
	current participant and bitrate. the rule is an upper limit.)
	*/
	Match ( numberParticipants:number, bitrate:number )
	{
		return numberParticipants <= this.numberParticipants && bitrate <= this.bitrate;
	}

	ActiveStreams () {
		// Amazon only supports a few permutations of the 3 streams active, and never more than 2.
		if ( this.lowStreamBitrate !== 0 && this.highStreamBitrate !== 0)
			return ActiveStreams.kHiAndLow;
		if (this.lowStreamBitrate !== 0 && this.mediumStreamBitrate !== 0)
			return ActiveStreams.kMidAndLow;
		if (this.lowStreamBitrate !== 0)
			return ActiveStreams.kLow;
		if (this.highStreamBitrate !== 0)
